...
if( $_SERVER[ "REQUEST_METHOD" ] == "GET" ){
  ...
  if( !empty( $_POST["sign-up"] ) ){
    ...
    if( empty( $todo__user ) ){//the user does not exist
      if( User::addUser( $username, $hashedPassword ) ){
        $todo__user = User::userExist( $username );
        session_start();
        $_SESSION[ "user_id" ] = $todo__user["id"];
        $_SESSION[ "username" ] = $todo__user["username"];
        //CREATING ROOT COLLECTION
        Collection::createRoot( "root", $todo__user["id"], "This is the root collection. A predefine collection with heightes level in the application");
        header( "Location:./mytodos.php" );exit;
      } else{
        header( "Location:./registration.php?msg=There+had+been+an+error+while+adding+the+user" );exit;
      }

    }else{
      header( "Location:./registration.php?msg=There+had+been+an+error+with+the+registration+.+The+user+exists" );exit;
    }
    /*if( User::addUser( $username, $hashedPassword ) ){
      //$todo__user = User::userExist( $username );
      if( !empty( $todo__user ) ){
