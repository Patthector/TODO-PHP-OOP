...
if( !empty( $user ) ){
	...
	if($_SERVER["REQUEST_METHOD"] == "GET"){
		...
		if(!empty($_GET["id"])){
			...
			if( !empty( $collection->get__collection_id() )){# if we have a collection, set the title heading with the Collection's name
				...
				if(!empty($_GET["action"])){
				  ...
				  exit;
					...
					$rootCollection = Collection::rootCollection( $id );//TRUE--FALSE
			...
			if(!empty($_GET["id_collection"])){
				$default_collection = filter_input(INPUT_GET, "id_collection", FILTER_SANITIZE_NUMBER_INT);
			}
